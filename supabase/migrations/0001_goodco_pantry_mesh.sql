...
create table if not exists pantry_memberships (
  ...
  role text not null default 'member'
    check (role in ('member', 'manager', 'network_admin')),
  created_at timestamptz not null default now(),
...
as $$
  select exists (
    ...
    where pm.pantry_id = target_pantry_id
      and pm.user_id = auth.uid()
  );
$$;

create or replace function public.is_approved_pantry_member(target_pantry_id uuid)
returns boolean
language sql
stable
security definer
...
create or replace function public.has_approved_pantry()
returns boolean
language sql
stable
security definer
...
on pantry_memberships for select
to authenticated
...
create policy "pantry members can view lots"
on inventory_lots for select
to authenticated
using (
  is_pantry_member(pantry_id)
  or exists (
    ...
    where ml.lot_id = inventory_lots.id and member_pantry.network_id = source_pantry.network_id
  )
);

create policy "pantry members can add lots"
on inventory_lots for insert
to authenticated
with check (is_pantry_member(pantry_id));

create policy "pantry members can update lots"
on inventory_lots for update
to authenticated
using (is_pantry_member(pantry_id))
with check (is_pantry_member(pantry_id));

create policy "pantry members can view movements"
...
using (
  exists (
    ...
    where l.id = inventory_movements.lot_id
      and is_pantry_member(l.pantry_id)
  )
);

create policy "pantry members can add movements"
...
create policy "members can read category mappings"
on category_mappings for select
to authenticated
using (
  scope = 'base_catalog'
  or (pantry_id is not null and is_pantry_member(pantry_id))
);

create policy "members can add local category mappings"
on category_mappings for insert
to authenticated
with check (
  scope = 'local'
  and pantry_id is not null
  and is_pantry_member(pantry_id)
);

create policy "authenticated users can add extraction jobs"
on extraction_jobs for insert
to authenticated
with check (true);

create policy "authenticated users can add classification events"
on classification_events for insert
to authenticated
with check (true);

create policy "approved pantries can browse listings"
on marketplace_listings for select
to authenticated
using (has_approved_pantry());

create policy "source pantry can add listings"
on marketplace_listings for insert
to authenticated
with check (is_approved_pantry_member(source_pantry_id));

create policy "source pantry can update listings"
on marketplace_listings for update
to authenticated
using (exists (select 1 from pantry_memberships where user_id = auth.uid() and pantry_id = source_pantry_id and role in ('manager', 'network_admin')))
with check (exists (select 1 from pantry_memberships where user_id = auth.uid() and pantry_id = source_pantry_id and role in ('manager', 'network_admin')));

create policy "approved pantries can view requests"
on marketplace_requests for select
to authenticated
using (
  is_pantry_member(requesting_pantry_id)
  or is_pantry_member(source_pantry_id)
);

create policy "approved pantries can add requests"
on marketplace_requests for insert
to authenticated
with check (is_approved_pantry_member(requesting_pantry_id));

create policy "member pantries can view transfers"
on marketplace_transfers for select
to authenticated
using (
  is_pantry_member(source_pantry_id)
  or is_pantry_member(destination_pantry_id)
);

create policy "approved pantries can read network policies"
on network_policies for select
to authenticated
using (has_approved_pantry());

create policy "network admins manage memberships"
on pantry_memberships for all
to authenticated
using (is_network_admin((select network_id from pantries where id = pantry_id)))
with check (is_network_admin((select network_id from pantries where id = pantry_id)));

create policy "network admins manage policies"
on network_policies for all
to authenticated
using (is_network_admin(network_id))
with check (is_network_admin(network_id));

create policy "network admins read policy decisions"
on marketplace_policy_decisions for select
to authenticated
using (is_network_admin(network_id));
