...
create table if not exists pantry_memberships (
  ...
  role text not null default 'staff'
    check (role in ('network_admin', 'admin', 'staff', 'volunteer')),
  created_at timestamptz not null default now(),
...
to authenticated
using (is_pantry_member(source_pantry_id))
with check (is_pantry_member(source_pantry_id));
