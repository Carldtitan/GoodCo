...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success || parsed.data.destinationPantryId !== activePantry.pantryId) return NextResponse.json({ error: "Only the receiving pantry can confirm this transfer." }, { status: 400 });
    const { data, error } = await createServiceRoleSupabaseClient().rpc("finalize_marketplace_transfer", toTransferRpc(parsed.data, activePantry.userId));
