...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "Check the cancellation details." }, { status: 400 });
    const { data, error } = await createServiceRoleSupabaseClient().rpc("cancel_marketplace_reservation", toCancellationRpc(parsed.data, activePantry.userId));
