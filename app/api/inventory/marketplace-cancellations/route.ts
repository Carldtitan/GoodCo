...
export async function POST(request: Request) {
  ...
  const { listingId, requestId, lotId, quantity, unit, reason } = parsed.data;
  const { error } = await supabase.rpc("cancel_marketplace_reservation", {
  ...
  if (error) {
    return NextResponse.json({ error: "Cancellation failed" }, { status: 409 });
  }

  return NextResponse.json({ result: { cancelled: true } });
}
